...
- **既定エンジン（全OS）: [Argos Translate](https://github.com/argosopentech/argos-translate)** —
  翻訳専用NMT。CPUのみ・1ペア約100〜200MB・低計算コストで即時・完全オフライン。
- **任意の小型LLM: MLX Gemma 4 E2B / E4B**（Apple Silicon）— UIで選択。公開済みの
  [`ToPo-ToPo/gemma-4-E2B-it-qat-mlx-4bit`](https://huggingface.co/ToPo-ToPo/gemma-4-E2B-it-qat-mlx-4bit)
  等を使用。**未キャッシュ時は確認ダイアログ→OK後にDL**（約3.3GB、以降オフライン）。
- **任意: llama.cpp（Gemma 4 GGUF）** — 既定では導入しません（`uv sync --extra llamacpp`）。
