...
- **任意の小型LLMエンジン: MLX（Gemma 4 E2B）** — Apple Silicon 専用。公式重みから
...
しません）。MLX は Apple Silicon で効率的に動きます。

```bash
# 一度だけ実行: 公式重みから MLX 4bit を変換してローカルに用意（約9.5GB DL→約3.3GB）
uv run python tools/build_mlx_model.py
```

作成後、UIのエンジン欄に **「Gemma 4 E2B (MLX)」** が現れます（メモリ約4〜5GB）。
未作成のうちは UI 上で「未作成」と理由が表示されます。変換と読み込みは同じ
**mlx-vlm 0.6.3** で行う必要があるため、本アプリはこの版に固定しています。

- モデルは `~/.cache/auto-translator/mlx/` に保存（アンインストールで削除されます）。
- `AUTO_TRANSLATE_MLX_MODEL` でローカルパスや別モデルに差し替え可能。
...
| `AUTO_TRANSLATE_MLX_MODEL` | `~/.cache/auto-translator/mlx/gemma-4-E2B-it-qat-mlx-4bit` | MLXモデルのローカルパス（or HFリポジトリID） |
...
engines/              プラガブルなバックエンド（argos / llamacpp / mlx）。遅延import
tools/build_mlx_model.py  公式重みからローカルMLXモデルを変換（Apple Silicon）
