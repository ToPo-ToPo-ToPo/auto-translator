...
## ソース変更の自動反映（auto-reload）

Dock から起動したまま開発できます。ソースを編集すると自動で反映されるので、
`.app` を Dock から入れ直す必要はありません。

- **Python（`*.py` / `pyproject.toml`）の変更:** `app.py` の監視スレッドが変更を
  検知して終了コード 87 で自己終了し、ランチャー（`auto-translator.app` 内の
  `launch`）の再起動ループが新しいコードで起動し直します。ウインドウは
...
- 旧バージョン（`/api/version` 未対応）のサーバが動いたまま Dock から起動した
  場合は、ランチャーが旧サーバを停止して新しいコードで起動し直します。
